...
import { 
    CheckCircle, Scan, WarningCircle, ClockCounterClockwise, Student, Clock, UserFocus
} from 'phosphor-react';
...
const FaceAttendance: React.FC<FaceAttendanceProps> = ({ schoolId, students, classes, profile }) => {
  ...
  const handleScan = (scannedData: string) => {
    const student = students.find(s => s.id === scannedData);
    if (student) {
      ...
      <div className="w-full max-w-[1920px] mx-auto flex flex-col lg:flex-row gap-6 min-h-[90vh]">
        ...
        <div className="flex-1 bg-white dark:bg-[#1e293b] border-2 border-slate-300 shadow-sm flex flex-col">
            <div className="bg-[#1e3a8a] text-white p-6 border-b-4 border-slate-900 flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-3xl font-black tracking-tight uppercase">Face Recognition</h1>
                    <p className="text-xs font-bold text-white/70 uppercase tracking-widest">AI-Powered Attendance System</p>
                </div>
            </div>

            <div className="p-8 flex-1 flex flex-col items-center justify-center">
                <div className="w-full max-w-2xl bg-slate-50 dark:bg-[#0f172a] border-4 border-slate-200 dark:border-[#1e293b] p-2 relative">
                    <SmartScanner 
                        onScan={handleScan} 
                        mode="face" 
                        referenceData={students.map(s => ({ id: s.id, photoURL: s.photoURL, name: s.name }))}
                    />
                    ...
                    <div className="absolute inset-0 pointer-events-none border-[20px] border-transparent">
                        <div className="w-full h-full border-2 border-blue-500/30 animate-pulse flex items-center justify-center">
                            <div className="w-64 h-64 border-2 border-blue-500/50 rounded-full" />
                        </div>
                    </div>
                </div>
                ...
                <div className="mt-8 text-center">
                    <div className="flex items-center gap-2 justify-center text-slate-400 font-bold uppercase tracking-widest text-xs">
                        <Scan size={16} />
                        Position face within the frame to scan
                    </div>
                </div>
            </div>
            ...
            {scannedStudent && (
                <div className="fixed top-24 right-10 z-[100] bg-emerald-600 text-white border-4 border-slate-900 shadow-[8px_8px_0px_#1e3a8a] p-6 animate-in slide-in-from-right-12 fade-in duration-300 w-96">
                    <div className="flex gap-6 items-center">
                        <div className="w-20 h-20 bg-white dark:bg-[#1e293b] border-4 border-slate-900 flex items-center justify-center text-slate-900 dark:text-white font-black text-2xl shrink-0 overflow-hidden">
                        ...
                        <div className="flex-1 overflow-hidden">
                            <div className="flex items-center gap-2 mb-1">
                                ...
                                <span className="text-xs font-black uppercase tracking-widest">Face Recognized</span>
                            </div>
        ...
        <div className="w-full lg:w-96 flex flex-col gap-6">
            ...
            <div className="bg-white dark:bg-[#1e293b] border-2 border-slate-300 p-6 shadow-sm">
                <div className="flex items-center gap-2 mb-6 border-b-2 border-slate-100 dark:border-[#334155] pb-4">
                    ...
                    <h2 className="text-sm font-black uppercase tracking-widest text-slate-900 dark:text-white">Last Recognized</h2>
                </div>

                {lastScannedStudent ? (
                    <div className="flex flex-col items-center text-center">
                        <div className="w-32 h-32 bg-slate-100 border-4 border-slate-200 dark:border-[#1e293b] flex items-center justify-center text-slate-500 dark:text-slate-400 font-black text-4xl mb-4 overflow-hidden">
                    ...
                    <div className="py-12 flex flex-col items-center text-slate-300">
                        ...
                        <p className="text-[10px] font-black uppercase tracking-widest">Waiting for face...</p>
                    </div>
                )}
            </div>
            ...
            <div className="bg-white dark:bg-[#1e293b] border-2 border-slate-300 p-6 shadow-sm flex-1">
                ...
                <div className="space-y-4">
                    ...
                    {scanHistory.length === 0 && (
                        <p className="text-center py-8 text-[10px] font-bold text-slate-400 uppercase tracking-widest italic">No faces recognized yet</p>
                    )}
                </div>
            </div>
        </div>
