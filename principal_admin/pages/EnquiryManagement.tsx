    ...
    <div className="font-sans text-slate-900 dark:text-white animate-in fade-in duration-300 pb-20 bg-slate-100 min-h-screen p-4 md:p-6">
      ...
      <div className="w-full max-w-[1920px] mx-auto bg-white dark:bg-slate-800 border-2 border-slate-300 shadow-sm flex flex-col min-h-[90vh]">
        ...
        <div className="p-8 space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-white dark:bg-slate-800 p-6 border-2 border-blue-900 shadow-sm flex flex-col justify-between h-32 relative">
                ...
                <div className="bg-white dark:bg-slate-800 p-6 border-2 border-emerald-600 shadow-sm flex flex-col justify-between h-32 relative">
                    <div className="flex justify-between items-start">
            ...
            {/* --- LIST TABLE --- */}
            <div className="bg-white dark:bg-slate-800 border-2 border-[#1e3a8a] shadow-sm flex flex-col">
                <div className="px-6 py-4 bg-[#1e3a8a] border-b-2 border-[#1e3a8a] flex flex-col md:flex-row justify-between items-center gap-4">
                    ...
                    <div className="relative w-full md:w-64 bg-white dark:bg-slate-800 p-1">
                         <input 
                            type="text" 
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        <thead>
                            <tr className="bg-slate-50 dark:bg-slate-800/50 text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest border-b-2 border-slate-200 dark:border-slate-700">
                        ...
                        <tbody className="divide-y-2 divide-slate-100">
                            {filteredEnquiries.map((enq) => (
                                <tr key={enq.id} className="group hover:bg-slate-50 dark:bg-slate-800/50 transition-colors">
                                    <td className="p-4">
                                        <div className="flex items-center gap-3">
                                            <div className="w-10 h-10 bg-slate-100 border-2 border-slate-200 dark:border-slate-700 flex items-center justify-center text-slate-500 dark:text-slate-400 font-black text-xs">
                                                {enq.parentName[0]}
                                            </div>
                                            <div>
                                    ...
                                    <td className="p-4">
                                        <span className="px-2 py-1 bg-slate-100 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 text-[10px] font-black uppercase">
                                    ...
                                    <td className="p-4 text-right">
                                        <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                            {enq.status !== 'Admitted' && (
                                                <button onClick={() => handleQuickStatusUpdate(enq, 'Admitted')} className="p-2 bg-white dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 text-emerald-600 hover:border-emerald-600 transition-colors" title="Mark Admitted">
                                                    <Check size={14} weight="bold"/>
                                                </button>
                                            )}
                                            <button onClick={() => handleEdit(enq)} className="p-2 bg-white dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 hover:border-blue-500 hover:text-blue-500 transition-colors">
                                                <PencilSimple size={14} weight="bold"/>
                                            </button>
                                            <button onClick={() => handleDelete(enq.id!)} className="p-2 bg-white dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 text-rose-500 hover:border-rose-500 transition-colors">
      ...
      {showModal && (
        <div className="fixed inset-0 z-[500] flex items-center justify-center p-4">
            ...
            <div className="bg-white dark:bg-slate-800 w-full max-w-lg border-4 border-slate-800 p-0 relative z-10 animate-in zoom-in-95 shadow-2xl h-[85vh] flex flex-col">
                ...
                <div className="p-8 overflow-y-auto custom-scrollbar space-y-6 flex-1">
                    <div className="space-y-4">
                        <h4 className="text-xs font-black text-slate-800 dark:text-slate-100 uppercase tracking-wide border-b-2 border-slate-100 dark:border-slate-800 pb-2 flex items-center gap-2">
                ...
                <div className="p-6 border-t-2 border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 flex gap-3 shrink-0">
