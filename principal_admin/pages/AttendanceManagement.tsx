...
const AttendanceManagement: React.FC<AttendanceManagementProps> = ({ 
  ...
  if (isMobile) {
    return (
      <div className="min-h-screen bg-white dark:bg-slate-800 pb-32 font-sans relative overflow-hidden">
        <div className="max-w-3xl mx-auto space-y-8 relative z-10">
          ...
          <div className="bg-white dark:bg-slate-800 p-6 rounded-none shadow-[0_10px_40px_-10px_rgba(30,58,138,0.1)] border-b border-[#D4AF37]/30 space-y-6 relative overflow-hidden">
            ...
            <div className="flex bg-[#FCFBF8] dark:bg-slate-900 p-1 rounded-2xl border border-[#D4AF37]/20 shadow-inner">
              <button 
          ...
          <div className="px-4 space-y-8">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="bg-white dark:bg-slate-800 p-5 rounded-3xl flex items-center gap-5 shadow-[0_8px_30px_-6px_rgba(107,29,47,0.08)] border border-[#D4AF37]/20 relative overflow-hidden group">
            ...
            <div className="space-y-6 relative z-0">
                          ...
                          <div className="flex items-center gap-5">
                            <div className="relative shrink-0">
                              {student.photoURL ? (
                                  ...
                                  <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-[#1e3a8a] to-[#172554] flex items-center justify-center text-white font-black text-2xl shrink-0 border-2 border-white dark:border-slate-700 shadow-lg">
                                      {student.name[0]}
                                  </div>
                              )}
                            </div>
                            <div className="min-w-0">
                              ...
                              <p className="text-[10px] font-bold text-[#D4AF37] uppercase tracking-widest mt-1">Roll No: {student.rollNo}</p>
                            </div>
                          </div>
                      ...
                      <div key={teacher.id} className={`${statusBg} p-6 rounded-3xl border ${statusBorder} shadow-[0_10px_40px_-10px_rgba(30,58,138,0.1)] flex flex-col gap-6 transition-all duration-300 relative overflow-hidden group`}>
                        ...
                        <div className="flex items-center justify-between relative z-10 pl-3">
                          <div className="flex items-center gap-5">
                            <div className="relative shrink-0">
                              {teacher.photoURL ? (
                                  <img src={teacher.photoURL} alt={teacher.name} className="w-16 h-16 rounded-2xl object-cover shadow-md border-2 border-white dark:border-slate-700" />
                              ) : (
                                  <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-[#1e3a8a] to-[#172554] flex items-center justify-center text-white font-black text-2xl shrink-0 border-2 border-white dark:border-slate-700 shadow-lg">
                                      {teacher.name[0]}
                                  </div>
              ...
              {((activeTab === 'student' && visibleCount < studentsInClass.length) || (activeTab === 'staff' && visibleCount < filteredTeachers.length)) && (
                <button 
                  ...
                  className="w-full py-4 bg-white dark:bg-slate-800 text-[#1e3a8a] dark:text-white rounded-2xl font-black text-sm hover:bg-slate-50 dark:hover:bg-slate-700 transition-all flex items-center justify-center gap-2 shadow-[0_8px_20px_rgba(30,58,138,0.1)] border-2 border-[#D4AF37]/30 active:scale-[0.98]"
                >
  ...
  return (
    <div className="font-sans text-slate-900 dark:text-white animate-in fade-in duration-300 pb-20 bg-slate-100 min-h-screen p-4 md:p-6">
      ...
      <div className="w-full max-w-[1920px] mx-auto bg-white dark:bg-slate-800 border-2 border-slate-300 shadow-sm flex flex-col min-h-[90vh]">
        ...
        {/* --- TABS --- */}
        <div className="flex border-b-2 border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 px-8 pt-4 gap-2 overflow-x-auto">
            <button 
                onClick={() => setActiveTab('student')}
                className={`flex items-center gap-2 px-6 py-3 font-black uppercase tracking-widest text-xs transition-all border-b-4 ${activeTab === 'student' ? 'border-[#1e3a8a] text-[#1e3a8a] bg-white dark:bg-slate-800' : 'border-transparent text-slate-500 hover:text-slate-800 dark:text-slate-100 hover:bg-slate-100'}`}
            >
            ...
            <button 
                onClick={() => setActiveTab('staff')}
                className={`flex items-center gap-2 px-6 py-3 font-black uppercase tracking-widest text-xs transition-all border-b-4 ${activeTab === 'staff' ? 'border-[#1e3a8a] text-[#1e3a8a] bg-white dark:bg-slate-800' : 'border-transparent text-slate-500 hover:text-slate-800 dark:text-slate-100 hover:bg-slate-100'}`}
            >
        ...
        {activeTab === 'student' ? (
          <div className="p-8 space-y-8 bg-white dark:bg-slate-800">
            ...
            {scannedStudent && (
                <div className="fixed top-6 right-6 z-[100] bg-white dark:bg-slate-800 border-2 border-emerald-500 shadow-[4px_4px_0px_#10b981] p-4 animate-in slide-in-from-right-8 fade-in duration-300 w-80">
                    <div className="flex gap-4 items-center">
                        <div className="w-16 h-16 bg-slate-100 border-2 border-slate-200 dark:border-slate-700 flex items-center justify-center text-slate-500 dark:text-slate-400 font-black text-xl shrink-0">
            ...
            <div className="bg-white dark:bg-slate-800 border-2 border-[#1e3a8a] shadow-sm flex flex-col md:flex-row items-center justify-between p-1 mt-6">
                <div className="flex gap-2 w-full md:w-auto p-1">
                   {/* Class Selector */}
                   <div className="flex items-center bg-slate-100 border-2 border-slate-200 dark:border-slate-700 p-1 w-full md:w-56 group focus-within:border-[#1e3a8a]">
                   ...
                   {/* Date Picker */}
                   <div className="flex items-center bg-slate-100 border-2 border-slate-200 dark:border-slate-700 p-1 w-full md:w-48 group focus-within:border-[#1e3a8a]">
                ...
                {/* Mode Selector */}
                <div className="flex gap-1 p-1 bg-slate-100 border-2 border-slate-200 dark:border-slate-700">
                    <button 
                ...
                <div className="w-full md:w-64 p-1">
                     <div className="flex items-center bg-slate-50 dark:bg-slate-800/50 border-2 border-slate-200 dark:border-slate-700 p-1 group focus-within:border-[#1e3a8a]">
                         <input 
                            type="text" 
            ...
            <div className="bg-slate-50 dark:bg-slate-800/50 p-6 min-h-[400px] border-2 border-slate-200 dark:border-slate-700 border-t-0 -mt-2">
                ...
                {(attendanceMode === 'qr' || attendanceMode === 'face') && lastScannedStudent && (
                    <div className="mb-8 bg-white dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 p-4 flex items-center gap-4 shadow-sm animate-in fade-in duration-300">
                        <div className="w-14 h-14 bg-slate-100 border-2 border-slate-200 dark:border-slate-700 flex items-center justify-center text-slate-500 dark:text-slate-400 font-black text-xl shrink-0">
                        ...
                        <div className="flex-1 overflow-hidden">
                            ...
                            <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider truncate mt-0.5">
                                Class: {classes.find(c => c.id === lastScannedStudent.classId)?.name || 'N/A'} • Roll No: {lastScannedStudent.rollNo}
                            </p>
                        </div>
                    </div>
                )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                  {studentsInClass.slice(0, visibleCount).map(student => {
                    const status = getStudentStatus(student.id);
                    return (
                      <div key={student.id} className="p-5 border-2 border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 flex flex-col justify-between group hover:border-[#1e3a8a] hover:shadow-[4px_4px_0px_#1e3a8a] transition-all relative">
                        <div className="flex justify-between items-start mb-4">
                            <div className="flex items-center gap-3">
                                <div className="w-10 h-10 bg-slate-100 border-2 border-slate-200 dark:border-slate-700 flex items-center justify-center text-slate-500 dark:text-slate-400 font-black text-xs shrink-0 overflow-hidden">
                                ...
                                <div>
                                  ...
                                  <p className="text-[10px] text-slate-400 font-bold uppercase mt-0.5 tracking-widest">{student.rollNo}</p>
                                </div>
                            </div>
                        </div>
                        
                        <div className="pt-3 border-t-2 border-slate-100 dark:border-slate-800 flex flex-col gap-2">
                            ...
                            {attendanceMode === 'manual' && (
                                <div className="flex gap-1 mt-2">
                                    <button onClick={() => handleMarkAttendance(student.id, 'Present')} className={`flex-1 py-1.5 text-[9px] font-black uppercase tracking-widest border-2 transition-all ${status === 'Present' ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-white dark:bg-slate-800 text-emerald-600 border-emerald-600 hover:bg-emerald-50'}`}>P</button>
                                    <button onClick={() => handleMarkAttendance(student.id, 'Absent')} className={`flex-1 py-1.5 text-[9px] font-black uppercase tracking-widest border-2 transition-all ${status === 'Absent' ? 'bg-rose-600 text-white border-rose-600' : 'bg-white dark:bg-slate-800 text-rose-600 border-rose-600 hover:bg-rose-50'}`}>A</button>
                                    <button onClick={() => handleMarkAttendance(student.id, 'Leave')} className={`flex-1 py-1.5 text-[9px] font-black uppercase tracking-widest border-2 transition-all ${status === 'Leave' ? 'bg-amber-500 text-white border-amber-500' : 'bg-white dark:bg-slate-800 text-amber-500 border-amber-500 hover:bg-amber-50'}`}>L</button>
                                </div>
                            )}
                        </div>
                      </div>
               ...
               {visibleCount < studentsInClass.length && (
                 <div className="mt-8 flex justify-center">
                   <button 
                     onClick={() => setVisibleCount(prev => prev + 30)}
                     className="px-8 py-3 bg-white dark:bg-slate-800 border-2 border-slate-300 text-slate-700 dark:text-slate-200 font-black uppercase tracking-widest text-xs hover:border-[#1e3a8a] hover:text-[#1e3a8a] transition-colors shadow-sm"
                   >
                     Show More
                   </button>
                 </div>
