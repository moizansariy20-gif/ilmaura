  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {resources.map(res => (
          <div key={res.id} className="bg-white dark:bg-[#1e293b] p-8 rounded-[3rem] border border-slate-200 dark:border-[#1e293b]/50 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all group relative">
             <div className="absolute top-6 right-6 flex gap-2 opacity-0 group-hover:opacity-100 transition-all">
                <button onClick={() => handleDelete(res.id)} className="p-3 bg-slate-50 dark:bg-[#0f172a] text-slate-400 hover:text-rose-600 rounded-2xl hover:bg-rose-50 transition-all shadow-sm"><Trash2 size={18}/></button>
            </div>
            ...
            <div className="flex items-center gap-2 mt-6 pt-6 border-t border-slate-100 dark:border-[#334155]">
        ...
        {resources.length === 0 && (
            <div className="col-span-full py-32 text-center bg-white dark:bg-[#1e293b] rounded-[4rem] border-4 border-dashed border-slate-100 dark:border-[#334155]">
      ...
      {showModal && (
        <div className="fixed inset-0 z-[500] flex items-center justify-center p-4">
          ...
          <div className="bg-white dark:bg-[#1e293b] w-full max-w-lg rounded-[4rem] p-6 md:p-12 relative z-10 shadow-2xl animate-in zoom-in-95 duration-300">
             <div className="flex justify-between items-center mb-12">
                ...
                <button onClick={() => setShowModal(false)} className="w-14 h-14 bg-slate-50 dark:bg-[#0f172a] text-slate-400 hover:text-slate-900 dark:text-white rounded-[1.5rem] flex items-center justify-center shadow-sm border border-slate-100 dark:border-[#334155] active:scale-90 transition-all"><X size={24}/></button>
             </div>
             <div className="space-y-8">
                <div className="space-y-3">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Resource Title</label>
                    <input type="text" value={form.title} onChange={e => setForm({...form, title: e.target.value})} className="w-full px-6 py-4 bg-white/80 dark:bg-[#1e293b]/80 border-2 border-slate-200 dark:border-[#1e293b] rounded-2xl font-bold text-slate-800 dark:text-slate-100" placeholder="e.g. Physics Chapter 1 Notes" />
                </div>
                <div className="space-y-3">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-3">Link URL</label>
                    <input type="url" placeholder="https://..." className="w-full px-8 py-6 bg-slate-50 dark:bg-[#0f172a] border-2 border-slate-50 rounded-[2rem] text-sm font-bold focus:border-indigo-100 outline-none transition-all" value={form.url} onChange={e => setForm({...form, url: e.target.value})} />
                </div>
                
                <div className="space-y-3">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Description</label>
                    <textarea value={form.description} onChange={e => setForm({...form, description: e.target.value})} className="w-full px-6 py-4 bg-white/80 dark:bg-[#1e293b]/80 border-2 border-slate-200 dark:border-[#1e293b] rounded-2xl text-sm h-24 resize-none" placeholder="Brief details about this resource..." />
                </div>
                
                <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-3">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-3">Assign to Class</label>
                        <select value={form.classId} onChange={e => setForm({...form, classId: e.target.value})} className="w-full px-4 py-4 bg-slate-50 dark:bg-[#0f172a] border-2 border-slate-50 rounded-xl font-bold outline-none cursor-pointer focus:border-indigo-100 appearance-none text-sm">
                    ...
                    <div className="space-y-3">
                        ...
                        <select value={form.subjectId} onChange={e => setForm({...form, subjectId: e.target.value})} className="w-full px-4 py-4 bg-slate-50 dark:bg-[#0f172a] border-2 border-slate-50 rounded-xl font-bold outline-none cursor-pointer focus:border-indigo-100 appearance-none text-sm">
